...
exports.init = function init(initialUser) {
  ...
  if (!dbExists) {
    if (!initialUser || !initialUser.nickname || !initialUser.password) {
      return Promise.reject(new Error('valid initial user has to be provided when database does not exist'));
    }

    if (!fs.existsSync(dataFolderPath)) {
      logger.info(`creating data folder at "${dataFolderPath}"...`);
